...
class RelativeShapeAnisotropy(Base):
	"""
	Class for analysis of relative shape anisotropies.
	"""
	...
	def __init__(self, run):
		...
		super(RelativeShapeAnisotropy, self).__init__(run)


	def getMPLAxesForValueAsFunctionOfTime(self):
		"""
		Returns a `matplotlib.axes.Axes` object that contains a plot of the
		relative shape anisotropy, with the horizontal axis showing the
		simulation time `t`, and the vertical axis showing the relative shape
		anisotropy at that point in time.
		"""
		...
		lines.append(_line)
		legendLabels.append("Relative Shape Anisotropy")

		axes.legend(lines, legendLabels)

		axes.set_title("Star Polymer Relative Shape Anisotropy")
		axes.set_xlabel("Simulation Time t")
		axes.set_ylabel("Relative Shape Anisotropy")
	...
	def _computeValueAsFunctionOfTime(self):
		"""
		Takes the raw simulation output, and computes the relative shape
		anisotropy as a function of time. The result is returned as an
		...
		while True:
			...
			ret[time] = particles.getRelativeShapeAnisotropy()
	...
	def _getCacheFilename(self):
		...
		return "relativeShapeAnisotropy.txt"
