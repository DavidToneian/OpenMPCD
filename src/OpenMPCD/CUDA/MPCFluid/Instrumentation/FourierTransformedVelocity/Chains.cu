...
Chains::Chains(
	...
	const MPCFluid::Base* const mpcFluid_)
		: Base(sim, devMemMgr, mpcFluid_, mpcFluid_->getParticleCount() / chainLength_),
...
void Chains::measure()
{
	...
	for(unsigned int i=0; i<k_n.size(); ++i)
	{
		...
		const Vector3D<std::complex<MPCParticleVelocityType> > result =
			DeviceCode::calculateVelocityInFourierSpace_chainMPCFluid(
				mpcFluid->getParticleCount() / chainLength,
				chainLength,
