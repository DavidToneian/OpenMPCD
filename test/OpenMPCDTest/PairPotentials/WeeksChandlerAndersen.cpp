...
template<typename T>
static void test_force(const T epsilon, const T sigma)
{
	...
	const WeeksChandlerAndersen<T> interaction(epsilon, sigma);
	const WeeksChandlerAndersen<Taylor> interaction_taylor(epsilon, sigma);
...
static void test_force()
{
	...
	for(std::size_t e = 0; e < sizeof(epsilon)/sizeof(epsilon[0]); ++e)
	{
		for(std::size_t s = 0; s < sizeof(sigma)/sizeof(sigma[0]); ++s)
			test_force<T>(epsilon[e], sigma[s]);
	}
}
SCENARIO(
...
template<typename T>
static void test_potential(const T epsilon, const T sigma)
{
	using namespace OpenMPCD;
	using namespace OpenMPCD::PairPotentials;
...
static void test_potential()
{
	...
	{
		for(std::size_t s = 0; s < sizeof(sigma)/sizeof(sigma[0]); ++s)
			test_potential<T>(epsilon[e], sigma[s]);
	}
}
SCENARIO(
...
template<typename T>
static void test_forceOnR1DueToR2(const T epsilon, const T sigma)
{
	using namespace OpenMPCD;
	using namespace OpenMPCD::PairPotentials;
...
static void test_forceOnR1DueToR2()
{
	...
	{
		for(std::size_t s = 0; s < sizeof(sigma)/sizeof(sigma[0]); ++s)
			test_forceOnR1DueToR2<T>(epsilon[e], sigma[s]);
	}
}
SCENARIO(
	"`OpenMPCD::PairPotentials::WeeksChandlerAndersen::forceOnR1DueToR2`",
	"")
{
	#define SEQ_DATATYPE (float)(double)(long double)(OpenMPCD::FP)
	#define CALLTEST(_r, _data, T) \
