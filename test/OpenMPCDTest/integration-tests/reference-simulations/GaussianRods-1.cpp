/**
 * @file
 * This test runs a simulation with a `GaussianRods`, and checks whether the
...
static const OpenMPCD::Configuration getConfiguration()
{
	...
	config.set("mpc.fluid.gaussianRods.meanBondLength", 0.9);
	config.set("mpc.fluid.gaussianRods.springConstant", 0.5);
	config.set("mpc.fluid.gaussianRods.mdStepCount", 3);
	config.set("instrumentation.gaussianRods.bondLengthHistogram.binCount", 10);
	config.set("instrumentation.gaussianRods.bondLengthHistogram.low", 0.0);
	config.set("instrumentation.gaussianRods.bondLengthHistogram.high", 10.0);
...
SCENARIO(
	"CUDA, GaussianRods, Reference Simulation 1",
	"[CUDA]"
	)
{
	static const char* const path =
		"test/data/reference-simulations/GaussianRods-1.vtf";
